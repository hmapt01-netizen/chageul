...
function getPostTitle() {
    ...
    return h1 ? h1.innerText.trim() : document.title.replace(' | 차를 쓰다', '').trim();
}

// 1. 로컬 캐시에서 즉시 불러오기 (0.001초 체감)
function getCachedComments(slug) {
    try {
        const saved = localStorage.getItem("chageul_comments_" + slug);
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (Array.isArray(list)) {
                ...
                try {
                    localStorage.setItem("chageul_comments_" + slug, JSON.stringify(list));
                } catch(e) {}
                return list;
            }
        }
    ...
    if (!comments || comments.length === 0) {
        listContainer.innerHTML = `
            <div style="text-align:center; padding:24px 16px; background:#f8fafc; border-radius:10px; color:#94a3b8; font-size:0.88rem; border:1px dashed #e2e8f0;">
                아직 등록된 댓글이 없습니다.<br>첫 번째 의견이나 소감을 남겨보세요!
            </div>
        `;
        return;
    }
            ...
            <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:12px; padding:16px 18px; box-shadow:0 2px 6px rgba(0,0,0,0.02); margin-bottom:12px;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                    <div style="display:flex; align-items:center; gap:10px;">
                        <div style="width:32px; height:32px; border-radius:50%; background:#f1f5f9; color:#0f172a; display:flex; align-items:center; justify-content:center; font-weight:800; font-size:0.85rem; flex-shrink:0;">${escapeHtml(initial)}</div>
                        <div>
...
async function handleCommentSubmit(e) {
    ...
    try {
        localStorage.setItem("chageul_comments_" + slug, JSON.stringify(currentList));
    } catch(e) {}
...
async function handleDeleteComment(id) {
    ...
    try {
        localStorage.setItem("chageul_comments_" + slug, JSON.stringify(updated));
    } catch(e) {}
